...
const LeftSidebarMenu: React.FC<LeftSidebarProps> = ({
  ...
  const escaladas = useEscaladasCount();
      ...
      <Box className="left-sidebar-menu">
        <Box className="logo">
          <Link href={`/${lang}/dashboard/`}>
            <Typography component={"span"} sx={{ fontWeight: 700, fontSize: "15px" }}>ComVida</Typography>
          </Link>
        </Box>
              ...
              if (item.type === "sub-title") {
                ...
                return (
                  ...
                  >
                    ...
                    {(() => {
                      const badge = item.href === "/sessoes/" && escaladas > 0
                        ? { text: String(escaladas), className: "bg-danger" }
                        : item.badge;
                      return badge ? (
                        <Typography
                          component={"span"}
                          className={`trezo-badge ${badge.className || ""}`}
                        >
                          {badge.text}
                        </Typography>
                      ) : null;
